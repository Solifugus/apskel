...
class Framework {
	// Framework Settings 
	protected $framework_version;  // current version of the framework to use

	// Request Settings
	protected $session_id;             // current session ID (from php or artificially maintained via cli)
	protected $request_uri;            // full URL of the current request
	protected $request_protocol;       // protocol used, e.g. http, https, cli
	protected $request_address;        // address used to reach here
	protected $request_path;           // path, e.g. /controller/request
	protected $request_query;          // query string appended to URI
	protected $request_session_id;     // identifies the current session
	protected $controller_name;
	protected $request_name;
		
	// Application Settings
	protected $application;            // name of application
	protected $environment;            // environment under version
	...
	// Database Settings
	protected $databases;                  // all environment's database settings
	...
	public function getControllerFileName( $param_controller_name = null ) {
		...
		return strtolower($controller_name) . '/' . $this->getClassifizedName( $controller_name ) . 'Controller.php';
	}

	public function getControllerRegistrationFileName( $param_controller_name = null ) {
		if( $param_controller_name == null ) { $controller_name = $this->getControllerName(); }
		else                                 { $controller_name = $param_controller_name; }
		return strtolower($controller_name) . '/' . $this->getClassifizedName( $controller_name ) . 'Registration.php';
	}

	public function getControllerClassName( $param_controller_name ) {
		return $this->getClassifizedName( $param_controller_name ) . 'Controller';
	}
	...
	public function getIdOfDatabaseFor( $param_usage = 'read_write' ) {
		...
		$options = array();
		$last = count( $this->databases );
		for( $id = 0; $id < $last; $id++ ) {
			if( strtolower($this->databases[$id]['usage']) === strtolower( $param_usage ) ) { $options[] = $id; }
		}
	...
	public function getDatabaseAddress( $id = 0 ) {
		return $this->databases[$id]['address'];
	}
	...
	public function __construct() {
		...
		$this->determineApplicationSettings();  // 
	...
	protected function determineApplicationSettings() {
		...
		foreach($GLOBALS['identifications'] as $identification) {
			$found = false;
			if( strpos( $this->request_address, $identification['recognizer'] ) !== false ) { 
				$found = true;
				$this->application = $identification['application'];
				$this->environment = $identification['environment'];
		...
		if( $found !== true ) {
			...
			print "Environment not known.."; exit;
		}

		// Load Application Settings
		$path_to_settings = "../environments/{$this->version}/{$this->environment}/settings.php";
		if( file_exists( $path_to_settings ) ) { 
			...
			print "Error: the \"$path_to_settings\" file is missing."; exit;
		}

		// Set Working Direction to Appropriate Base
		chdir( "../application/{$this->version}" );
	}
	...
	protected function determineSessionVariables() {
		...
		if (!isset($_SESSION['user_name'])) {
			...
			$_SESSION['controller'] = $this->getUriController( $this->settings['landing_page'] ); // controller last set as session default
			$_SESSION['request']    = $this->getUriRequest( $this->settings['landing_page'] );    // request last se as session default

			// current session output format, unless overridden
			if ($this->request_protocol == 'cli') {
	...
	protected function determineRequestDetails($param_show = false) {
		...
		if ( $this->request_path > '' ) {
			$uri_parameters = explode('/', $this->request_path);
	...
	public function getView( $controller_name = null, $request_name = null, $parameters = null ) { 
		...
		if( $controller_name == 'resource' ) {
			...
			$file_name = trim( strtolower( substr( $this->request_path, strlen( "resource/$controller_name/" ) ) ), '/' );
		...
		if( $registration === null ) {
			...
			$request_is_fatal_reason .= "There is no registration file for the \"$controller_name\" controller (\"~/application/$this->version/$registration_file_name\"). "; 
		}
		...
		if( !file_exists( $controller_file_name ) ) {
			$request_is_fatal = true;
			$request_is_fatal_reason = "The \"$controller_name\" controller has no associated programming code (~/application/$this->version/$controller_file_name). ";
		}
		else {
			...
			if( !class_exists( $controller_class_name ) ) {
				$request_is_fatal = true;
				$request_is_fatal_reason = "The \"$controller_name\" controller's class is not defined in its code file (~/application/$this->version/$controller_file_name). ";
			}
			else {
				...
				if( !method_exists( $controller, $request_method_name ) ) {
					$request_is_fatal = true;
					$request_is_fatal_reason = "The \"$controller_name\" controller's \"$request_method_name\" method is not defined in the code (~/application/$this->version/$controller_file_name). ";
				}
			}
...
EndOfCSS;
	...
	// Converts displayable name to class-name convention
...
EndOfSQL;
	...
	public function getListOfControllers( $param_version = null ) {
		if( $param_version == null ) { $version = $this->version; }
	...
	private function loadControllerRegistration( $param_controller ) {
		...
		if( !file_exists( $registration_file_name ) ) { return null; }
	...
	public function getResourceFile( $file_name, $controller ) {
		...
		$environment_resource_path = "resources-{$this->environment}/" . $file_name;
		...
		if( $resource_type == 'css' ) {
			...
			if( file_exists( $environment_resource_path ) ) {
				$found = true;
				$resource_content .= "/* NOTE -- Found \"$file_name\" in the {$this->environment} environment's global application area (\"$environment_resource_path\"; this overrides the controller and main global application areas) *" . "/\n" . file_get_contents( $environment_resource_path ) . "\n\n";; 
			}
			else { $resource_content .= "/* Resource \"$file_name\" does not exist as in the {$this->environment} environment's global application area ($environment_resource_path). *" . "/\n"; }
